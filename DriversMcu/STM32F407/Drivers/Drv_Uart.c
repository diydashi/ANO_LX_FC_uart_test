...
void NewU2DataCheck()
{
	...
	while(U2RxInCnt!=U2RxoutCnt)
	{
		//U2GetOneByte(U2RxDataTmp[U2RxoutCnt++]);	//原处理方式
		UartUserData.U2Buffer[Uart2DataCnt++] = U2RxDataTmp[U2RxoutCnt++];	//从环形缓冲区中扣出所有有效数据 放进Uart2UserDataBuffer
		
		if(U2RxoutCnt >= 100)
			U2RxoutCnt = 0;
	}
	
	if(UartUserData.U2Buffer[0] == 0xAA && UartUserData.U2Buffer[1] == 0xFF)	//校验帧头 和 是否为用户广播数据
	{
		if(DataFrameCheck(UartUserData.U2Buffer))	//数据校验通过
		{
			Uart2UserDataHandle();
		}
	}
	memset(UartUserData.U2Buffer, 0x00, sizeof(UartUserData.U2Buffer));	//清空缓冲区
}
...
void NewU3DataCheck()
{
	...
	while(U3RxInCnt!=U3RxoutCnt)
	{
		//U3GetOneByte(U3RxDataTmp[U3RxoutCnt++]);	//原处理方式
		UartUserData.U3Buffer[Uart3DataCnt++] = U3RxDataTmp[U3RxoutCnt++];	//从环形缓冲区中扣出所有有效数据 放进Uart3UserDataBuffer
		
		if(U3RxoutCnt >= 100)
			U3RxoutCnt = 0;
	}
	
	if(UartUserData.U3Buffer[0] == 0xAA && UartUserData.U3Buffer[1] == 0xFF)	//校验帧头 和 是否为用户广播数据
	{
		if(DataFrameCheck(UartUserData.U3Buffer))	//数据校验通过
		{
			Uart3UserDataHandle();
		}
	}
	memset(UartUserData.U3Buffer, 0x00, sizeof(UartUserData.U3Buffer));	//清空缓冲区
}
